...
def check_status(space, x, y, z):
    ...
    count = space[max(x-1, 0): min(x_max, x+2), max(y-1, 0): min(y_max, y+2), max(z-1, 0): min(z_max, z+2)].sum()
    if space[x,y,z] == 0:
        if count == 3:
            return 1
        else:
...
def check_status_4d(space, x, y, z, w):
    ...
    count = space[max(x-1, 0): min(x_max, x+2), max(y-1, 0): min(y_max, y+2), max(z-1, 0): min(z_max, z+2), max(w-1, 0): min(w_max, w+2)].sum()
    if space[x,y,z,w] == 0:
        if count == 3:
            return 1
        else:
